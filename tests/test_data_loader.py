...
class TestTinkoffDataLoader:
    def test_missing_token_raises_on_list(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        loader = TinkoffDataLoader(auth_token=None)
    ...
    def test_explicit_token_wins_over_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        loader = TinkoffDataLoader(auth_token="explicit-token")
        assert loader.is_configured is True

    def test_sandbox_env_used_when_sandbox(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("TINKOFF_SANDBOX_TOKEN", "sandbox-token")
        monkeypatch.setenv("TINKOFF_REAL_TOKEN", "real-token")
        loader = TinkoffDataLoader(sandbox=True)
        assert loader._token == "sandbox-token"

    def test_real_env_used_when_not_sandbox(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("TINKOFF_SANDBOX_TOKEN", "sandbox-token")
        monkeypatch.setenv("TINKOFF_REAL_TOKEN", "real-token")
        loader = TinkoffDataLoader(sandbox=False)
        assert loader._token == "real-token"

    def test_list_tickers_parses_response(self, monkeypatch: pytest.MonkeyPatch) -> None:
        handlers = [
            _FakeResponse(
                _tinkoff_instruments(
                    [
                        {
                            "ticker": "SBER",
                            "figi": "BBG004730N88",
        ...
        loader = TinkoffDataLoader(
            auth_token="t", sandbox=True, session=session, rate_per_sec=600.0  # type: ignore[arg-type]
        )
        tickers = loader.list_tickers()
        assert len(tickers) == 1
        assert tickers[0].figi == "BBG004730N88"

    def test_auth_rejected_raises_typed_exception(self, monkeypatch: pytest.MonkeyPatch) -> None:
        handlers = [_FakeResponse({}, status_code=401)]
        session = _FakeSession(handlers)
        loader = TinkoffDataLoader(
            auth_token="bad", sandbox=True, session=session, rate_per_sec=600.0  # type: ignore[arg-type]
        )
        with pytest.raises(LoaderAuthError):
            loader.list_tickers()

    def test_iter_ohlcv_yields_rows(self) -> None:
        handlers = [
            _FakeResponse(
                _tinkoff_instruments(
                    [
    ...
    def test_iter_corporate_actions_yields_dividends(self) -> None:
        handlers = [
            _FakeResponse(
                _tinkoff_instruments(
                    [
            ...
            _FakeResponse(
                _tinkoff_dividends(
                    [
                        {
                            "figi": "BBG004730N88",
                            "lastBuyDate": "2026-05-15T00:00:00Z",
                            "dividend": 33.4,
                        }
                    ]
                )

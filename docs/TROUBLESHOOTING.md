...
| 15 | Grafana datasource `Prometheus` пропала | `curl -u admin:*** http://grafana:3000/api/datasources` → 401 или пусто | Portainer обрезал `PROVISIONING_DATASOURCES_YML_B64` env var (issue #283/#297) | PR #300: bind-mount provisioning/dashboards из репо. После StackUpdate datasource появляется автоматически. |
| 16 | `prometheus.yml` empty → no scrape targets | `curl http://prom:9090/api/v1/targets` → `[]` | Portainer обрезал `PROM_YML_B64` env var (issue #283) | PR #284: bind-mount `prometheus.yml` из репо. |

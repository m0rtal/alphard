...
### Backfilled drift
- **PRs #385, #388, #391** — pre-existing changelog drift surfaced by the drift-window guard during cycle160 review of open PRs (#394, #399). These PRs were merged to `main` between the last changelog-touching commit (PR #386) and cycle160 review; they were simply not referenced here yet. None are part of the open PRs (#394 alphard-web v2 / #399 Grafana removal) — this entry is a backfill, not an additional change. Restores the `tests/test_changelog_drift_window.py::test_drift_window_prs_are_all_in_changelog` gate to green.

### Fixed
- **`scripts/quickstart.sh` — refuse to run when `alphard-*` containers exist under a different compose project** (Closes #382, PR #385). Pre-fix, `quickstart.sh` ran `docker compose up -d` unscoped while `docker-compose.yaml` pins `container_name:` for the six long-running services. On a host that already held a literal `alphard-bot` (or any sibling) owned by a different compose project, compose aborted with `Conflict. The container name "/alphard-bot" is already in use by container "a83aebe16bb8..."`. Quickstart is the only documented bootstrap path for a new operator, so that message was a dead end. New precheck (mirroring PR #380's pattern in `pre_pr_smoke.sh`) detects foreign-owned `alphard-*` containers up front and exits with the owning project name and a teardown command. Operator sees the actionable answer in 1 s instead of fighting the conflict. (PR #385.)
- **`scripts/backup_database.py` — refuse empty/truncated `pg_dump` before writing and pruning** (Closes #387, PR #388). Pre-fix, `run_backup()` accepted any `pg_dump` that exited `0`, gzipped it, wrote it as the newest daily backup, then pruned the retention window — `_prune()` ranks files by timestamp and never inspects content, so an empty or truncated dump could win the daily window and evict a genuinely-valid older backup. With the default `--daily-keep 7` on cron, **seven silently-broken runs would delete every real recovery point** — no error logged anywhere. New `_dump_rejection_reason(sql)` gate rejects payloads missing pg_dump's `--format=plain` completion trailer (`-- PostgreSQL database dump complete`), returning `RC_DUMP_REJECTED=3` *before* the write and *before* the prune, so existing backups survive. `tests/test_backup_database.py` fixtures updated to wrap bodies in `_valid_dump()` so they mirror real pg_dump output instead of weakening the gate. 4 new tests (rejects empty, rejects truncated, rejected run does NOT prune existing backups, accepts complete). Behaviour change on rc: a broken dump now returns `3` instead of `0` — strictly safer than silent deletion. (PR #388.)
- **`scripts/ci_local.sh` + `scripts/pre_pr_smoke.sh` — lint `scripts/` to match CI parity** (Closes #389, PR #391). Pre-fix, local gates ran flake8/black only on `src/ tests/`, but `.github/workflows/ci.yml` runs them on `src/ tests/ scripts/`. The gap let a formatting violation slip through PR #388 and forced a follow-up style() commit. `ci_local.sh` now includes `scripts/` in both invocations; `pre_pr_smoke.sh` gains step `[2.5/4]` that runs flake8 + black on `src/ tests/ scripts/` between the bot health check and pytest, with explicit exit 2 on violation (matches pytest failure RC). Mirrors CI exactly so a violation that would fail CI also fails the local smoke gate. (PR #391.)
...
### Changed
- **Two Heartbeat panels removed from `docker/grafana/dashboards/alphard-phase28.json`** — "Heartbeat rate (5m window)" and "Heartbeat lag (alerts if > 60s)" (PR #367). Both were uninformative: "Heartbeat rate" measures `alphard_heartbeats_total[5m]`, which increments once every 60 s whenever the heartbeat loop runs at all and therefore says nothing about whether the system is healthy; "Heartbeat lag" measures `time() - alphard_heartbeat_last_tick_timestamp`, the same signal as the "Alphard uptime" panel (`alphard_uptime_seconds`), which is **kept**. The surviving panels (Alphard uptime, Universe size, Tickers with full history, Daily-candle row accumulation) cover the operational signals needed at a glance. No behaviour change inside `alphard-bot`: the dashboard JSON is bind-mounted read-only, served directly by Grafana, and Portainer stack-updates only touch `docker-compose.yaml`. Operators lose two panels after a pull — that is intentional. `tests/test_dashboard_heartbeat_cleanup.py` (5 tests) pins the post-cleanup panel set so the panels cannot creep back in. (PR #367.)
